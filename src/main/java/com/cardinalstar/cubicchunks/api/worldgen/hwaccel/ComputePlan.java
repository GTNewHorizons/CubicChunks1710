...
public class ComputePlan {
    ...
    public BufferDescriptor describeBuffer(KernelSubmissionToken submission, BufferDataType dataType, int lenX, int lenY) {
        return describeBuffer(submission, dataType, lenX, lenY, 1);
    }

    public BufferDescriptor describeBuffer(KernelSubmissionToken submission, BufferDataType dataType, int lenX, int lenY, int lenZ) {
    ...
    public <Key> Map<String, BufferDescriptor> submit(KernelExecutor<Key> executor, Key key, Map<String, BufferDescriptor> inputs) {
        int id = nextSubmission++;

        int level = inputs.values().stream().mapToInt(d -> d.submission().level()).max().orElse(-1) + 1;
